...
let tiles;
let score = 0;
let best = Number(localStorage.getItem('meme-2048-best') || 0);
let hasWon = false;
...
function move(direction) {
    ...
    if (before === next.join(',')) {
        statusElement.textContent = 'No meme combo there. Try another direction.';
        return;
    }
    ...
    if (score > best) {
        best = score;
        localStorage.setItem('meme-2048-best', best);
    }
    render();

    if (tiles.includes(2048) && !hasWon) {
        hasWon = true;
        statusElement.textContent = 'You reached the ultimate meme tile!';
    } else if (!canMove()) {
        statusElement.textContent = 'Game over. Start a new round and keep the memes flowing.';
    }
}
