...
function renderInventory() {
    ...
    document.querySelectorAll('[data-give-all]').forEach(button => button.addEventListener('click', () => { const input = document.querySelector(`[data-give-amount="${button.dataset.giveAll}"]`); input.value = input.max; }));
    document.querySelectorAll('[data-give]').forEach(button => button.addEventListener('click', async () => { const product = products.find(item => item.id === Number(button.dataset.give)); const input = document.querySelector(`[data-give-amount="${button.dataset.give}"]`); const amount = Math.floor(Number(input.value)); if (!product || amount < 1 || amount > product.stock) return; if (!confirm(`Give ${amount} ${product.name} to Kelsey and remove it from stock?`)) return; const newStock = product.stock - amount; const { error } = await supabase.from('products').update({ stock: newStock }).eq('id', product.id); if (error) { alert(`Stock could not be updated: ${error.message}`); return; } product.stock = newStock; renderProducts(); renderInventory(); }));
...
document.querySelector('#product-form').addEventListener('submit', async event => { event.preventDefault(); if (currentRole !== 'owner') return; const image = document.querySelector('#product-image').files[0]; let imageUrl = null; if (image) { if (!image.type.startsWith('image/')) { alert('Please choose a PNG, JPG, or WEBP image.'); return; } const safeName = image.name.replace(/[^a-z0-9.-]/gi, '-').toLowerCase(); const id = crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2)}`; const path = `${id}-${safeName}`; const upload = await supabase.storage.from('product-images').upload(path, image, { upsert: false, contentType: image.type }); if (upload.error) { alert(`Image upload failed: ${upload.error.message}. Run the latest supabase-schema.sql and make sure the product-images bucket exists.`); return; } imageUrl = supabase.storage.from('product-images').getPublicUrl(path).data.publicUrl; } const product = { name: document.querySelector('#product-name').value, price: Number(document.querySelector('#product-price').value), stock: Number(document.querySelector('#product-stock').value), color: document.querySelector('#product-color').value, description: 'A fresh print from the Preston studio.', image_url: imageUrl }; const { data, error } = await supabase.from('products').insert(product).select().single(); if (error) { alert(`Product could not be added: ${error.message}. Run the latest supabase-schema.sql so products has an image_url column.`); return; } products.push(data); event.target.reset(); renderProducts(); renderInventory(); });
